...
private fun HsvPicker(
    ...
    Canvas(
        ...
        val handle = Textures.WIDGET_COLOR_PICKER_HANDLE_CHOICE
        val offset = Offset(
        ...
        drawTexture(
            texture = handle,
            dstRect = Rect(
                offset = offset - (handle.size / 2).toSize(),
                size = handle.size.toSize(),
            ),
        )
    }
}

private val hollowHandleDrawable = DrawableSet(
    normal = Textures.WIDGET_COLOR_PICKER_HOLLOW_HANDLE,
    focus = Textures.WIDGET_COLOR_PICKER_HOLLOW_HANDLE_HOVER,
...
private fun OverlaySlider(
    ...
    val sliderDrawable = LocalSliderDrawable.current
