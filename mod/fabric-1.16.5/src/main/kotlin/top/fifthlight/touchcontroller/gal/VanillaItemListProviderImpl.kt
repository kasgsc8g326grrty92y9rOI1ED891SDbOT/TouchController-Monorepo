...
object VanillaItemListProviderImpl : VanillaItemListProvider {
    class CreativeTabImpl(group: ItemGroup, items: PersistentList<ItemStack>) :
        ...
        override val type = when (group) {
            ItemGroup.INVENTORY -> VanillaItemListProvider.CreativeTab.Type.SURVIVAL_INVENTORY
            ItemGroup.SEARCH -> VanillaItemListProvider.CreativeTab.Type.SEARCH
            else -> VanillaItemListProvider.CreativeTab.Type.CATEGORY
        }

        override val name = TextImpl(group.translationKey)
    ...
    private val creativeTabs by lazy {
        ItemGroup
            .GROUPS
            .filter { it != ItemGroup.HOTBAR }
            .map {
                val list = DefaultedList.of<ItemStack>()
                Registry.ITEM.forEach { item -> item.appendStacks(it, list) }
